...
[Người dùng / Kế toán]
 ...
 (Xác thực User)                 │ (REST API qua HTTPS, Token từ Cognito)
                                 ▼
                         ┌───────────────┐
                         │ Elastic       │ ◄── [Backend: .NET 9 Web API]
                         │ Beanstalk     │
                                 │
    ┌────┼───────────────┬───────┴─────────┐
    ▼    ▼               ▼                 ▼
 ┌─────┐ ┌─────────┐   ┌───────────────┐   ┌───────────────┐
 │ RDS │ │ S3      │   │ECS Fargate    │   │ Systems Mgr   │
 └─────┘ └─────────┘   └───────────────┘   └───────────────┘
...
### 2.2. Backend Hosting: AWS Elastic Beanstalk

- **Tech stack của bạn**: .NET 9 API. (Team đã có Dockerfile/docker-compose).
- **Tại sao chọn AWS Elastic Beanstalk**:
  - Hoạt động ổn định, tự động hóa provisioning EC2, load balancing (ALB), và Auto Scaling.
  - Phù hợp với kiến trúc .NET API Monolith. Quản lý Auto Scaling Group tự động duy trì 2 EC2 instances trên 2 AZs đảm bảo High Availability nhưng vẫn dùng máy chủ chi phí hợp lý (t3).
...
- **Yêu cầu của bạn**: _Lưu trữ và quản lý hóa đơn tốt, TỐI ƯU CHI PHÍ._
- **Tại sao chọn**:
...
- **Chiến lược TỐI ƯU CHI PHÍ CHUYÊN SÂU**:
  - Khai báo **S3 Lifecycle Rules**. Hoá đơn 1-3 vạn cái sau một thời gian sẽ rất tốn kém. S3 Lifecycle cho phép cấu hình:
    - _0 - 90 ngày đầu_: Lưu ở S3 Standard (truy xuất tức thì, giá chuẩn).
    - _Sau 90 ngày_: Tự động chuyển data sang **Amazon S3 Glacier Instant Retrieval** hoặc **Glacier Deep Archive** (Giá rẻ hơn tới 90%, cực kì tối ưu cho hóa đơn cũ chỉ lưu để đối phó thanh tra).
...
- **Tuần 7**: Triển khai hệ thống lên AWS. Frontend lên Amplify, Backend lên Elastic Beanstalk. DB đưa lên RDS. API OCR lên AWS ECS Fargate.
